...
RemoteKeyCode   DATA    46H ;

B_TX1_Busy  BIT     20H.0   ; 发送忙标志
TX1_Cnt     DATA    47H     ; 发送计数
RX1_Cnt     DATA    48H     ; 接收计数
RX1_Buffer  DATA    50H     ; 50 ~ 6FH 接收缓冲
SEND_BUFFER DATA    70H     ; 70 ~ 71H
...
;=================== 检测是否收到串口信息 ===================
    MOV     A, TX1_Cnt
    ; CJNE 假如A != RX1_Cnt, 跳转到L_ReturnData
...
L_KeyProcess:
	...
	; [modified by wkcn] 通过串口1发送键码
	MOV R0, #SEND_BUFFER
    MOV @R0, KeyCode
    ; 通过串口发送键码
	LCALL F_SendDataString1
...
L_Quit_KeyProcess:
    ...
    INC     hour        ;if(RemoteKeyCode == 17)  hour +1
    MOV     A, hour
    CLR     C
    SUBB    A, #24
    JC      L_K17_Display
...
;========================================================================
; 函数: F_SendDataString1
; 描述: 串口1发送动态的字符串函数。
; 参数: R0: 字符串首地址.
; 返回: none.
;========================================================================
F_SendDataString1:
    MOV    A, @R0          ;Get current char
    JZ      L_SendDataEnd1      ;Check the end of the string
    SETB    B_TX1_Busy      ;标志发送忙
    MOV     SBUF, A         ;发送一个字节
    JB      B_TX1_Busy, $   ;等待发送完成
    INC     R0              ;increment string ptr
    SJMP    F_SendDataString1       ;Check next
L_SendDataEnd1:
    RET
